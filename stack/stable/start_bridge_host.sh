...
PIX_STEER_ABS_LIMIT="${PIX_CARLA_STEER_ABS_LIMIT:-}"
PIX_STEER_TAU="${PIX_CARLA_STEER_TAU:-}"
...
PIX_BRAKE_CREEP_THROTTLE="${PIX_CARLA_BRAKE_CREEP_THROTTLE:-}"
PIX_BRAKE_CREEP_MAX_BRAKE_CMD="${PIX_CARLA_BRAKE_CREEP_MAX_BRAKE_CMD:-}"
PIX_BRAKE_CREEP_SPEED_THRESHOLD_MPS="${PIX_CARLA_BRAKE_CREEP_SPEED_THRESHOLD_MPS:-}"
PIX_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS="${PIX_CARLA_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS:-}"
PIX_SUPPRESS_BRAKE_BELOW_TARGET="${PIX_CARLA_SUPPRESS_BRAKE_BELOW_TARGET:-}"
PIX_BRAKE_TARGET_SPEED_MARGIN_MPS="${PIX_CARLA_BRAKE_TARGET_SPEED_MARGIN_MPS:-}"
PIX_TARGET_SPEED_BRAKE_MAX_CMD="${PIX_CARLA_TARGET_SPEED_BRAKE_MAX_CMD:-}"
...
PIX_STEERING_REPORT_SIGN="${PIX_CARLA_STEERING_REPORT_SIGN:-}"
PIX_ACTUATION_STEER_STATUS_SIGN="${PIX_CARLA_ACTUATION_STEER_STATUS_SIGN:-}"
PIX_ROS_Y_SIGN="${PIX_CARLA_ROS_Y_SIGN:-}"
PIX_SENSOR_QUEUE_TIMEOUT_SEC="${PIX_CARLA_SENSOR_QUEUE_TIMEOUT_SEC:-}"
PIX_PHYSICS_MASS_KG="${PIX_CARLA_PHYSICS_MASS_KG:-}"
PIX_PHYSICS_DRAG_COEFFICIENT="${PIX_CARLA_PHYSICS_DRAG_COEFFICIENT:-}"
PIX_PHYSICS_CENTER_OF_MASS_X_M="${PIX_CARLA_PHYSICS_CENTER_OF_MASS_X_M:-}"
PIX_PHYSICS_CENTER_OF_MASS_Y_M="${PIX_CARLA_PHYSICS_CENTER_OF_MASS_Y_M:-}"
PIX_PHYSICS_CENTER_OF_MASS_Z_M="${PIX_CARLA_PHYSICS_CENTER_OF_MASS_Z_M:-}"
PIX_PHYSICS_TIRE_FRICTION="${PIX_CARLA_PHYSICS_TIRE_FRICTION:-}"
PIX_PHYSICS_WHEEL_DAMPING_RATE="${PIX_CARLA_PHYSICS_WHEEL_DAMPING_RATE:-}"
PIX_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG="${PIX_CARLA_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG:-}"
PIX_PHYSICS_REAR_MAX_STEER_ANGLE_DEG="${PIX_CARLA_PHYSICS_REAR_MAX_STEER_ANGLE_DEG:-}"
PIX_PHYSICS_USE_SWEEP_WHEEL_COLLISION="${PIX_CARLA_PHYSICS_USE_SWEEP_WHEEL_COLLISION:-}"
...
if [[ -n "${PIX_STEER_ABS_LIMIT}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_STEER_ABS_LIMIT=$(shell_quote "${PIX_STEER_ABS_LIMIT}")"
fi
if [[ -n "${PIX_STEER_TAU}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_STEER_TAU=$(shell_quote "${PIX_STEER_TAU}")"
fi
...
if [[ -n "${PIX_BRAKE_CREEP_THROTTLE}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_BRAKE_CREEP_THROTTLE=$(shell_quote "${PIX_BRAKE_CREEP_THROTTLE}")"
fi
if [[ -n "${PIX_BRAKE_CREEP_MAX_BRAKE_CMD}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_BRAKE_CREEP_MAX_BRAKE_CMD=$(shell_quote "${PIX_BRAKE_CREEP_MAX_BRAKE_CMD}")"
fi
if [[ -n "${PIX_BRAKE_CREEP_SPEED_THRESHOLD_MPS}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_BRAKE_CREEP_SPEED_THRESHOLD_MPS=$(shell_quote "${PIX_BRAKE_CREEP_SPEED_THRESHOLD_MPS}")"
fi
if [[ -n "${PIX_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS=$(shell_quote "${PIX_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS}")"
fi
if [[ -n "${PIX_SUPPRESS_BRAKE_BELOW_TARGET}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_SUPPRESS_BRAKE_BELOW_TARGET=$(shell_quote "${PIX_SUPPRESS_BRAKE_BELOW_TARGET}")"
fi
if [[ -n "${PIX_BRAKE_TARGET_SPEED_MARGIN_MPS}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_BRAKE_TARGET_SPEED_MARGIN_MPS=$(shell_quote "${PIX_BRAKE_TARGET_SPEED_MARGIN_MPS}")"
fi
if [[ -n "${PIX_TARGET_SPEED_BRAKE_MAX_CMD}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_TARGET_SPEED_BRAKE_MAX_CMD=$(shell_quote "${PIX_TARGET_SPEED_BRAKE_MAX_CMD}")"
fi
...
if [[ -n "${PIX_STEERING_REPORT_SIGN}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_STEERING_REPORT_SIGN=$(shell_quote "${PIX_STEERING_REPORT_SIGN}")"
fi
if [[ -n "${PIX_ACTUATION_STEER_STATUS_SIGN}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_ACTUATION_STEER_STATUS_SIGN=$(shell_quote "${PIX_ACTUATION_STEER_STATUS_SIGN}")"
fi
...
if [[ -n "${PIX_PHYSICS_MASS_KG}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_MASS_KG=$(shell_quote "${PIX_PHYSICS_MASS_KG}")"
fi
if [[ -n "${PIX_PHYSICS_DRAG_COEFFICIENT}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_DRAG_COEFFICIENT=$(shell_quote "${PIX_PHYSICS_DRAG_COEFFICIENT}")"
fi
if [[ -n "${PIX_PHYSICS_CENTER_OF_MASS_X_M}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_CENTER_OF_MASS_X_M=$(shell_quote "${PIX_PHYSICS_CENTER_OF_MASS_X_M}")"
fi
if [[ -n "${PIX_PHYSICS_CENTER_OF_MASS_Y_M}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_CENTER_OF_MASS_Y_M=$(shell_quote "${PIX_PHYSICS_CENTER_OF_MASS_Y_M}")"
fi
if [[ -n "${PIX_PHYSICS_CENTER_OF_MASS_Z_M}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_CENTER_OF_MASS_Z_M=$(shell_quote "${PIX_PHYSICS_CENTER_OF_MASS_Z_M}")"
fi
if [[ -n "${PIX_PHYSICS_TIRE_FRICTION}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_TIRE_FRICTION=$(shell_quote "${PIX_PHYSICS_TIRE_FRICTION}")"
fi
if [[ -n "${PIX_PHYSICS_WHEEL_DAMPING_RATE}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_WHEEL_DAMPING_RATE=$(shell_quote "${PIX_PHYSICS_WHEEL_DAMPING_RATE}")"
fi
if [[ -n "${PIX_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG=$(shell_quote "${PIX_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG}")"
fi
if [[ -n "${PIX_PHYSICS_REAR_MAX_STEER_ANGLE_DEG}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_REAR_MAX_STEER_ANGLE_DEG=$(shell_quote "${PIX_PHYSICS_REAR_MAX_STEER_ANGLE_DEG}")"
fi
if [[ -n "${PIX_PHYSICS_USE_SWEEP_WHEEL_COLLISION}" ]]; then
  PIX_BRIDGE_EXPORT_CMD+=" && export PIX_CARLA_PHYSICS_USE_SWEEP_WHEEL_COLLISION=$(shell_quote "${PIX_PHYSICS_USE_SWEEP_WHEEL_COLLISION}")"
fi
...
echo "PIX steer abs limit: ${PIX_STEER_ABS_LIMIT}"
echo "PIX steer tau: ${PIX_STEER_TAU}"
...
echo "PIX brake creep throttle: ${PIX_BRAKE_CREEP_THROTTLE}"
echo "PIX brake creep max brake cmd: ${PIX_BRAKE_CREEP_MAX_BRAKE_CMD}"
echo "PIX brake creep speed threshold mps: ${PIX_BRAKE_CREEP_SPEED_THRESHOLD_MPS}"
echo "PIX brake creep min target velocity mps: ${PIX_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS}"
echo "PIX suppress brake below target: ${PIX_SUPPRESS_BRAKE_BELOW_TARGET}"
echo "PIX brake target speed margin mps: ${PIX_BRAKE_TARGET_SPEED_MARGIN_MPS}"
echo "PIX target speed brake max cmd: ${PIX_TARGET_SPEED_BRAKE_MAX_CMD}"
...
echo "PIX steering report sign: ${PIX_STEERING_REPORT_SIGN}"
echo "PIX actuation steer status sign: ${PIX_ACTUATION_STEER_STATUS_SIGN}"
echo "PIX ROS y sign: ${PIX_ROS_Y_SIGN}"
echo "PIX sensor queue timeout sec: ${PIX_SENSOR_QUEUE_TIMEOUT_SEC}"
echo "PIX physics mass kg: ${PIX_PHYSICS_MASS_KG}"
echo "PIX physics drag coefficient: ${PIX_PHYSICS_DRAG_COEFFICIENT}"
echo "PIX physics center of mass x m: ${PIX_PHYSICS_CENTER_OF_MASS_X_M}"
echo "PIX physics center of mass y m: ${PIX_PHYSICS_CENTER_OF_MASS_Y_M}"
echo "PIX physics center of mass z m: ${PIX_PHYSICS_CENTER_OF_MASS_Z_M}"
echo "PIX physics tire friction: ${PIX_PHYSICS_TIRE_FRICTION}"
echo "PIX physics wheel damping rate: ${PIX_PHYSICS_WHEEL_DAMPING_RATE}"
echo "PIX physics front max steer angle deg: ${PIX_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG}"
echo "PIX physics rear max steer angle deg: ${PIX_PHYSICS_REAR_MAX_STEER_ANGLE_DEG}"
echo "PIX physics use sweep wheel collision: ${PIX_PHYSICS_USE_SWEEP_WHEEL_COLLISION}"
...
if [[ "$EXECUTE" -eq 1 ]]; then
  ...
  if [[ -n "${PIX_STEER_ABS_LIMIT}" ]]; then
    export PIX_CARLA_STEER_ABS_LIMIT="${PIX_STEER_ABS_LIMIT}"
  fi
  if [[ -n "${PIX_STEER_TAU}" ]]; then
    export PIX_CARLA_STEER_TAU="${PIX_STEER_TAU}"
  fi
  if [[ -n "${PIX_THROTTLE_GAIN}" ]]; then
  ...
  if [[ -n "${PIX_BRAKE_CREEP_THROTTLE}" ]]; then
    export PIX_CARLA_BRAKE_CREEP_THROTTLE="${PIX_BRAKE_CREEP_THROTTLE}"
  fi
  if [[ -n "${PIX_BRAKE_CREEP_MAX_BRAKE_CMD}" ]]; then
    export PIX_CARLA_BRAKE_CREEP_MAX_BRAKE_CMD="${PIX_BRAKE_CREEP_MAX_BRAKE_CMD}"
  fi
  if [[ -n "${PIX_BRAKE_CREEP_SPEED_THRESHOLD_MPS}" ]]; then
    export PIX_CARLA_BRAKE_CREEP_SPEED_THRESHOLD_MPS="${PIX_BRAKE_CREEP_SPEED_THRESHOLD_MPS}"
  fi
  if [[ -n "${PIX_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS}" ]]; then
    export PIX_CARLA_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS="${PIX_BRAKE_CREEP_MIN_TARGET_VELOCITY_MPS}"
  fi
  if [[ -n "${PIX_SUPPRESS_BRAKE_BELOW_TARGET}" ]]; then
    export PIX_CARLA_SUPPRESS_BRAKE_BELOW_TARGET="${PIX_SUPPRESS_BRAKE_BELOW_TARGET}"
  fi
  if [[ -n "${PIX_BRAKE_TARGET_SPEED_MARGIN_MPS}" ]]; then
    export PIX_CARLA_BRAKE_TARGET_SPEED_MARGIN_MPS="${PIX_BRAKE_TARGET_SPEED_MARGIN_MPS}"
  fi
  if [[ -n "${PIX_TARGET_SPEED_BRAKE_MAX_CMD}" ]]; then
    export PIX_CARLA_TARGET_SPEED_BRAKE_MAX_CMD="${PIX_TARGET_SPEED_BRAKE_MAX_CMD}"
  fi
  if [[ -n "${PIX_SPEED_GUARD_MAX_MPS}" ]]; then
  ...
  if [[ -n "${PIX_STEERING_REPORT_SIGN}" ]]; then
    export PIX_CARLA_STEERING_REPORT_SIGN="${PIX_STEERING_REPORT_SIGN}"
  fi
  if [[ -n "${PIX_ACTUATION_STEER_STATUS_SIGN}" ]]; then
    export PIX_CARLA_ACTUATION_STEER_STATUS_SIGN="${PIX_ACTUATION_STEER_STATUS_SIGN}"
  fi
  if [[ -n "${PIX_ROS_Y_SIGN}" ]]; then
  ...
  if [[ -n "${PIX_PHYSICS_MASS_KG}" ]]; then
    export PIX_CARLA_PHYSICS_MASS_KG="${PIX_PHYSICS_MASS_KG}"
  fi
  if [[ -n "${PIX_PHYSICS_DRAG_COEFFICIENT}" ]]; then
    export PIX_CARLA_PHYSICS_DRAG_COEFFICIENT="${PIX_PHYSICS_DRAG_COEFFICIENT}"
  fi
  if [[ -n "${PIX_PHYSICS_CENTER_OF_MASS_X_M}" ]]; then
    export PIX_CARLA_PHYSICS_CENTER_OF_MASS_X_M="${PIX_PHYSICS_CENTER_OF_MASS_X_M}"
  fi
  if [[ -n "${PIX_PHYSICS_CENTER_OF_MASS_Y_M}" ]]; then
    export PIX_CARLA_PHYSICS_CENTER_OF_MASS_Y_M="${PIX_PHYSICS_CENTER_OF_MASS_Y_M}"
  fi
  if [[ -n "${PIX_PHYSICS_CENTER_OF_MASS_Z_M}" ]]; then
    export PIX_CARLA_PHYSICS_CENTER_OF_MASS_Z_M="${PIX_PHYSICS_CENTER_OF_MASS_Z_M}"
  fi
  if [[ -n "${PIX_PHYSICS_TIRE_FRICTION}" ]]; then
    export PIX_CARLA_PHYSICS_TIRE_FRICTION="${PIX_PHYSICS_TIRE_FRICTION}"
  fi
  if [[ -n "${PIX_PHYSICS_WHEEL_DAMPING_RATE}" ]]; then
    export PIX_CARLA_PHYSICS_WHEEL_DAMPING_RATE="${PIX_PHYSICS_WHEEL_DAMPING_RATE}"
  fi
  if [[ -n "${PIX_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG}" ]]; then
    export PIX_CARLA_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG="${PIX_PHYSICS_FRONT_MAX_STEER_ANGLE_DEG}"
  fi
  if [[ -n "${PIX_PHYSICS_REAR_MAX_STEER_ANGLE_DEG}" ]]; then
    export PIX_CARLA_PHYSICS_REAR_MAX_STEER_ANGLE_DEG="${PIX_PHYSICS_REAR_MAX_STEER_ANGLE_DEG}"
  fi
  if [[ -n "${PIX_PHYSICS_USE_SWEEP_WHEEL_COLLISION}" ]]; then
    export PIX_CARLA_PHYSICS_USE_SWEEP_WHEEL_COLLISION="${PIX_PHYSICS_USE_SWEEP_WHEEL_COLLISION}"
  fi
