...
class StartBridgeHostTests(unittest.TestCase):
    def test_pix_vehicle_leaves_wheel_steer_guard_unset_by_default(self) -> None:
        ...
        self.assertIn("PIX throttle gain: 2.6", proc.stdout)
        self.assertIn("PIX min throttle: 0.0", proc.stdout)
        self.assertIn("PIX max throttle: 0.75", proc.stdout)
        self.assertIn("PIX brake gain: 0.002", proc.stdout)
        self.assertIn("PIX max brake: 0.008", proc.stdout)
        self.assertIn("PIX brake deadband: 0.05", proc.stdout)
        self.assertNotIn("export PIX_CARLA_SKIP_WHEEL_STEER_ANGLE", proc.stdout)
        self.assertIn("export PIX_CARLA_THROTTLE_GAIN=2.6", proc.stdout)
        self.assertIn("export PIX_CARLA_MIN_THROTTLE=0.0", proc.stdout)
        self.assertIn("export PIX_CARLA_MAX_THROTTLE=0.75", proc.stdout)
        self.assertIn("export PIX_CARLA_BRAKE_GAIN=0.002", proc.stdout)
        self.assertIn("export PIX_CARLA_MAX_BRAKE=0.008", proc.stdout)
