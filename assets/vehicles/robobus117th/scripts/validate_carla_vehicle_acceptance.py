...
def wheel_payload(wheel: Any, index: int) -> dict[str, float | int]:
    return {
        "index": index,
        "radius_cm": round_float(getattr(wheel, "radius", 0.0)),
        "width_cm": round_float(getattr(wheel, "width", 0.0)),
...
def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        checks = {
            ...
            "wheel_width_match": all(within(float(w["width_cm"]), EXPECTED_WHEEL_WIDTH_CM, 2.0) for w in wheels),
