...
当前方案的边界也要明确：这是“单 CARLA actor + PIX 视觉 + 稳定车辆物理”的可测试路径，已经不是外部跟随体；但 collision/PhysicsAsset 仍沿用稳定 carrier。2026-04-25 复查发现旧 runtime 的 CARLA `bounding_box.extent` 为 `x=0.0, y=1.046, z=0.1`，会污染 Autoware object bridge / CARLA API 可见车辆尺寸。仓库已补 `assets/vehicles/robobus117th/patches/carla_0_9_15_robobus_bbox_override.patch`，用于在 CARLA actor 注册前覆盖 `vehicle.pixmoving.robobus` 的 API bbox；正式 runtime shipping 二进制已替换并通过 strict bbox 动态验收，bbox extents 为 `x=1.91, y=0.955, z=1.1045`。该补丁不等同于完成原生 PIX PhysicsAsset。如果验收要求完全原生 PIX collision、wheel bone、PhAT 和可视化轮胎动画，需要继续做 UE4 PhAT/C++ 资产 authoring。

2026-05-11 复查确认 CARLA 0.9.15 的 `FWheelPhysicsControl` / RPC `WheelPhysicsControl` 不携带 wheel width 字段，因此 Python runtime 中 `wheel.width=0` 不是有效的车辆资产失败证据。随后使用 UE4 source inspect 验证 `BP_Robobus117th_FLW/FRW/RLW/RRW` 的 `shape_width=25.0`，vehicle movement 也正确引用这四个 wheel class。40km/h 闭环跑偏、终点前停止等 L1/L2 现象仍应按仿真保真问题继续排查，但不能用 runtime wheel width 作为归因。
...
- `scenarios/l0/robobus117th_vehicle_blueprint_acceptance.yaml` 能把 bbox/collision、轮胎半径/轴距/轮距/转角和传感器挂载写入 KPI；wheel width 需通过 UE4 source inspect 验证，不能从 CARLA 0.9.15 runtime API 判断。
...
```bash
simctl run \
  --scenario scenarios/l0/robobus117th_vehicle_blueprint_acceptance.yaml \
  --run-root runs/robobus117th_vehicle_blueprint \
  --slot stable-slot-01 \
  --execute
...
独立 CARLA 动态验收，适合停掉 Autoware/CARLA bridge 后单独运行：

```bash
python3 assets/vehicles/robobus117th/scripts/validate_carla_vehicle_acceptance.py \
  --host 127.0.0.1 \
  --port 2000 \
  --run-dir runs/robobus_vehicle_acceptance \
  --actor-id vehicle.pixmoving.robobus \
  --strict-bbox
```

CARLA 0.9.15 源码 bbox 覆盖补丁，适合在公司 Ubuntu 主机的 source toolchain 中应用：

```bash
cd /home/pixmoving/PIX-Simulation-Validation-Platform
assets/vehicles/robobus117th/scripts/apply_robobus_bbox_override_to_carla.sh \
  --carla-root /home/pixmoving/zmf_ws/source_toolchain/carla-0.9.15 \
  --dry-run
...
补丁后的低内存构建建议优先用 UBT `-ForceUnity`，避免 `CarlaUE4Editor.Target.cs` 默认 `bUseUnityBuild=false` 触发超长逐文件编译：

```bash
cd /home/pixmoving/zmf_ws/source_toolchain/carla-0.9.15
export UE4_ROOT=/home/pixmoving/zmf_ws/source_toolchain/UnrealEngine-4.26
nice -n 10 ionice -c2 -n7 \
  bash "$UE4_ROOT/Engine/Build/BatchFiles/Linux/RunMono.sh" \
  "$UE4_ROOT/Engine/Binaries/DotNET/UnrealBuildTool.exe" \
  CarlaUE4Editor Linux Development \
  -project="/home/pixmoving/zmf_ws/source_toolchain/carla-0.9.15/Unreal/CarlaUE4/CarlaUE4.uproject" \
  -MaxParallelActions=2 \
  -ForceUnity
```

2026-04-25 packaged runtime 验证结果：

- runtime binary：`/home/pixmoving/CARLA_0.9.15/CarlaUE4/Binaries/Linux/CarlaUE4-Linux-Shipping`
- rollback backup：`/home/pixmoving/CARLA_0.9.15/CarlaUE4/Binaries/Linux/CarlaUE4-Linux-Shipping.zmf-bbox-bak-20260425T042812Z`
- packaged validation run：`/home/pixmoving/PIX-Simulation-Validation-Platform/runs/manual_robobus_bbox_override_packaged_20260425T042845Z`
- strict gate：`packaged_strict_vehicle_acceptance/vehicle_acceptance_summary.json`
- result：`passed=true`，`bbox_extent_m={x:1.91,y:0.955,z:1.1045}`，`distance_m=20.467825`，`peak_speed_mps=5.080839`，`speed_after_brake_mps=0.010979`
- screenshot：`packaged_direct_visual_validation/pix_robobus_direct_after_drive.png`

2026-05-11 wheel blueprint source inspect 结果：

- script：`assets/vehicles/robobus117th/unreal/inspect_robobus_wheel_blueprints.py`
- host log：`/tmp/pix_inspect_robobus_wheels_capture_20260511T155254Z.log`
- source result：四个 wheel class 均为 `shape_radius=32.3`、`shape_width=25.0`，前轮 `steer_angle=28.991`，后轮 `steer_angle=0.0`
- runtime API boundary：CARLA 0.9.15 runtime `get_physics_control()` 不输出 wheel width，不能把 `width=0` 当作 cooked asset 失败

2026-05-11 L0 runtime blueprint acceptance 结果：

- run：`/data/pix/sim_runs/robobus_blueprint_api_boundary_fix_20260511T160059Z/20260511T160059359468Z__robobus117th_vehicle_blueprint_acceptance`
- status：`passed`
- gate：`robobus117th_vehicle_blueprint_acceptance`
- pass：blueprint found、ego actor seen、bbox `1.91/0.955/1.1045m`、wheel radius、wheelbase、front/rear tread、front/rear steer limits、6 cameras、5 lidars
- diagnostic：`robobus_min_wheel_width_cm=0.0` 仅说明 CARLA 0.9.15 runtime API 不暴露 width，不参与 acceptance
...
- 当前可运行路径使用稳定 carrier 物理，适合推动 CARLA/Autoware 基础联调和视觉验证；若后续 KPI 依赖 40km/h 规控、精确碰撞体、轮胎动画或真实二轮/四轮转向模型，需要继续验证 movement tuning、center of mass、actuation bridge、wheel steer feedback、路线/地图一致性。
...
- Owner：`zmf`
- 当前可执行：先跑 `scenarios/l0/robobus117th_vehicle_blueprint_acceptance.yaml` 与 direct actor gate，确认 runtime 可观测几何和传感器仍通过；wheel width 另用 UE4 source inspect 留证。
- 下一步：继续围绕 40km/h 跑偏做 movement tuning、质心/阻尼、bridge 转角反馈和 route/map 一致性验证，而不是把 `wheel.width=0` 当作修复目标。
